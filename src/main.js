...
controls.screenSpacePanning = false; // pan orthogonal to screen when false
controls.minDistance = 10;
controls.maxDistance = 2000;
controls.maxPolarAngle = Math.PI * 0.9; // allow looking near top but not under ground

// Improve camera/controls defaults for a map-to-3D workflow
controls.rotateSpeed = 0.6;
controls.zoomSpeed = 1.2;
controls.panSpeed = 0.8;
controls.minPolarAngle = 0.1; // avoid looking exactly from the horizon
controls.minDistance = 5;
controls.maxDistance = 5000;
// Configure which mouse buttons do what: left=rotate, middle=dolly (zoom), right=pan (we'll override pan for forward/back)
...
renderer.domElement.addEventListener('pointerdown', (e) => {
    ...
    if (hits.length > 0) {
        ...
        // set controls target to the object's center so left-rotation orbits it
